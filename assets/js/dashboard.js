    ...
    projects: [
        {
            ...
            code: 'งา',
            name: 'งานสารสนเทศ',
            owner: 'ศรีนัดดาวนที: ศิริวิดี บานปะพงศ์',
            total: 120000,
            used: 100000,
            progress: 50,
            accent: '#06b6d4'
...
function mountDashboard(pageContent) {
    ...
    if (!template || document.getElementById('projectList')) return;
...
function initAuthAndData() {
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        let appUser = { name: user.email, role: 'staff' };
        try {
            const userDocSnap = await getDoc(doc(db, 'users', user.uid));
            if (userDocSnap.exists()) appUser = userDocSnap.data();
        } catch (error) {
...
async function initDashboardListener() {
    try {
        ...
        onSnapshot(DASHBOARD_REF, (docSnap) => {
                ...
                <div class="mt-4 space-y-3">
                    <div>
                        <div class="flex justify-between text-xs text-slate-500 dark:text-slate-400 mb-1">
                            <span>การใช้งบประมาณ</span><span>${usedPercent}%</span>
                        </div>
                        <div class="progress-track"><div class="progress-fill bg-amber-400" style="width:${clamp(usedPercent)}%"></div></div>
                    </div>
                    <div>
                        <div class="flex justify-between text-xs text-slate-500 dark:text-slate-400 mb-1">
                            <span>ความคืบหน้างาน</span><span>${progress.toFixed(1)}%</span>
                        </div>
...
function chartBaseOptions(stacked) {
    ...
    return {
        ...
        plugins: {
            ...
            tooltip: { callbacks: { label: (ctx) => `${ctx.dataset.label}: ${Number(ctx.raw).toLocaleString('th-TH')}` } }
        },
        scales: {
...
function baht(value) { return `฿${Number(value || 0).toLocaleString('th-TH')}`; }
function sum(items, key) { return items.reduce((acc, item) => acc + Number(item[key] || 0), 0); }
function percent(part, total) { return total > 0 ? ((Number(part || 0) / Number(total)) * 100).toFixed(1) : '0.0'; }
